...
temint_df <- tibble(date = dato_interior$Fecha,
                    ...
                    z = dato_interior$`Humedad (%)`,
                    u = dato_exterior$`Temperatura (ºC)`,
...
temint_df$tetrend <- decompose(temperatura_exterior_ts)$trend
temint_df$tetrend_sqr <- temint_df$tetrend ^ 2

temint_df$hitrend <- decompose(humedad_interior_ts)$trend
temint_df$hitrend_sqr <- temint_df$hitrend ^ 2

temint_df$hetrend <- decompose(humedad_exterior_ts)$trend
temint_df$hetrend_sqr <- temint_df$hetrend ^ 2
