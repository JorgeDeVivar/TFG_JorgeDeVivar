...
# RESULTADO: Podemos modelar los residuos del modelo md2 con un arima
#####################################
# Modelo alternativo, empleando la función tslm para incluir la estacionaridad
# como una variable explicativa categórica

temperatura_interior_ts <- ts(data = dato_interior$`Temperatura (ºC)`[1:unidades], start = 1, frequency = 3*24)
h <- 627
tem_int_split <- ts_split(temperatura_interior_ts, sample.out = h)
train_tem_int_ts <- tem_int_split$train
test_tem_int_ts <- tem_int_split$test

md3 <- tslm(train_tem_int_ts ~ season + trend + I(trend^2))
summary(md3)
checkresiduals(md3) # Los residuos claramente NO son ruido blanco

# Solución intentar modelar los residuos del modelo md2 con un arima
res_md3 <- residuals(md3)
model_residuals_md3 <- auto.arima(res_md3) # Result: ARIMA(2,1,2) with drift
summary(model_residuals_md3)
res_model_residuals_md3 <- residuals(model_residuals_md3)
acf(res_model_residuals_md3) # Más o menos limpio
pacf(res_model_residuals_md3) # Más o menos limpio
checkresiduals(model_residuals_md3)
################################################################################

############################ ARIMA Univariante DNE #############################################
plot(temperatura_interior_ts) # Media variando: Se recomienda diferenciar
acf(diff(temperatura_interior_ts))
pacf(diff(temperatura_interior_ts))
# Con una diferenciación los residuos son prácticamente ruido blanco.
...
# Modelo I1
ti_I1 <- Arima(ti_train,order=c(0,1,0))
summary(ti_I1)
res_ti_I1 <- residuals(ti_I1)
acf(res_ti_I1)
pacf(res_ti_I1) # Los residuos tienen estructura moving average
checkresiduals(ti_I1) # Los residuos tienen estructura moving average
...
ti_auto_md1 <- auto.arima(ti_train,
                          ...
                          parallel = TRUE)
summary(ti_auto_md1)
res_ti_auto_md1 <- residuals(ti_auto_md1)
acf(res_ti_auto_md1)
pacf(res_ti_auto_md1)
checkresiduals(ti_auto_md1)

plot(forecast(ti_auto_md1, h =18))
accuracy(ti_auto_md1)

ti_test_auto_md1 <- forecast(ti_auto_md1, h = 627)
...
temint_dfMLAuto$lag12 <- stats::lag(temint_dfMLAuto$y, n = 72)
...
x_no_NAs <- na.remove(x)

trainingdata <- window(x_no_NAs , end = time(x_no_NAs )[nrow(x_no_NAs)-627])
testdata<- window(x_no_NAs , start = time(x_no_NAs )[628])


summary(VAR(na.remove(trainingdata), p = 1, type = 'both'))

VARselect(na.remove(trainingdata), lag.max = 10, type = "both")

par(mar=c(1,1,1,1))
summary(fit <- VAR(na.remove(trainingdata), p = 2, type = "both"))
acf(resid(fit), 52)
...
for(i in 1:4){
  fc <- structure(list(mean=fit.pr$fcst[[i]][,"fcst"], x=trainingdata[,i],
